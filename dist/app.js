const products=[
  {id:1,name:'Fone Pulse Pro',desc:'Cancelamento de ruído e 40h de bateria',price:649.9,category:'audio',icon:'🎧',badge:'-20%'},
  {id:2,name:'Teclado Core 75',desc:'Mecânico, compacto e sem fio',price:459.9,category:'setup',icon:'⌨️',badge:'NOVO'},
  {id:3,name:'Sound Mini',desc:'Som potente e proteção contra água',price:289.9,category:'audio',icon:'🔊',badge:'DESTAQUE'},
  {id:4,name:'Mouse Flow X',desc:'Precisão ajustável e design ergonômico',price:219.9,category:'setup',icon:'🖱️',badge:null}
];
...
$('#checkoutBtn').addEventListener('click',()=>{closeCart();$('#checkoutDialog').showModal()});$('#dialogClose').addEventListener('click',()=>$('#checkoutDialog').close());$('#finishBtn').addEventListener('click',()=>{cart=[];renderCart();$('#checkoutDialog').close();showToast('Pedido de teste concluído')});
$('#newsletterForm').addEventListener('submit',e=>{e.preventDefault();e.target.reset();showToast('Cadastro de demonstração realizado!')});
